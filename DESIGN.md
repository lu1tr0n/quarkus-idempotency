# quarkus-idempotency — Design Document
...
2026-06-12: `quarkiverse/quarkus-idempotency-key` and `quarkus-idempotency` both 404, and core
...
- **`cache`** (default): Quarkus Cache / Caffeine, single-node. TTL via
  `quarkus.cache.caffeine."quarkus-idempotency".expire-after-write`. Atomic via mapping
...
```
quarkus-idempotency-parent (pom, quarkiverse-parent:NN)
...
└── deployment/   io.quarkiverse.idempotency.deployment
    └── IdempotencyProcessor
        ├── FeatureBuildItem("quarkus-idempotency")
